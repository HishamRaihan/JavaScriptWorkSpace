'use strict'
// all values need to be unique and they need to be comparable
// This will create a binary search tree all values to the left less then the current value and all values to the right will be greater then the current value
const BinaryTree = function () {
  this.root = undefined
  // this.nodeCount = 0
}

// this allows us to create new nodes for our binary tree
const Node = function (value) {
  // value is the data that the node keeps track of
  this.value = value
  // left will be a node whos value is less than the current nodes value
  this.left = undefined
  // right will be a node whos value is greater than the current nodes value
  this.right = undefined
}

BinaryTree.prototype.includes = function (value) {
  // if we dont have a root node we will return false
  if (!this.root) {
    // since we dont have any elements
    return false
  }
