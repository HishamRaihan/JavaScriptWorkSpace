...
function fetchPosts() {
	sendHttpRequest('GET', 'http://localhost:4741/destinations')
		.then((responseData) => {
			const listOfPosts = responseData.destinations;
			console.log(listOfPosts);
			console.log(listOfPosts);
			for (const post of listOfPosts) {
				const postEl = document.importNode(postTemplate.content, true);
				postEl.querySelector('h2').textContent = post.place.toUpperCase();
				postEl.querySelector('p').textContent = post.monthTraveled;
